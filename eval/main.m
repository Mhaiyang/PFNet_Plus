...
for i = 1:numel(algorithms)
    ...
    for j = 1:numel(datasets)
        dataset      = datasets{j};
%         predpath     = ['/media/iccd/disk2/cos_results/' alg '/' dataset '/'];
        predpath     = ['/media/iccd/disk/COS/cos_results/' alg '/' dataset '/'];
%         predpath     = ['/home/iccd/cos/extension/final/results/' alg '/' dataset '/'];
%         maskpath     = ['/home/iccd/data/NEW/test/' dataset '/mask/'];
%         maskpath     = ['/media/iccd/disk/COS//NEW/test/' dataset '/mask/'];
%         maskpath     = ['/media/iccd/disk/COS/data/NEW/test/' dataset '/mask/'];
        maskpath     = ['/media/iccd/disk/NEW/test/' dataset '/mask/'];
        if ~exist(predpath, 'dir'), continue; end

%         names = dir(['/home/iccd/data/NEW/test/' dataset '/mask/*.png']);
%         names = dir(['/media/iccd/disk1/NEW/test/' dataset '/mask/*.png']);
%         names = dir(['/media/iccd/disk/COS/data/NEW/test/' dataset '/mask/*.png']);
        names = dir(['/media/iccd/disk/NEW/test/' dataset '/mask/*.png']);
        names = {names.name};
